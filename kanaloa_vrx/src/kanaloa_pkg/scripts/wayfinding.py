...
class WAMV_Way_Point:
	...
	def check_station_keep_timer(self):
		...
		if self.distance <= 5 and self.station_keep_end_time_set == False and len(self.desired_coordinates) > 0 and self.navigation_indicator == True: 
	...
	def way_point_response_func(self, data):
		...
		try:
				...
				# lat = data.poses.pose.position.latitude
				# lon = data.poses.pose.position.longitude
				# minutes = 0
				...
				print("way point response failed")
